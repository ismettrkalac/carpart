...
return [
    ...
    */
    ...
    'meilisearch' => [
        ...
        'index-settings' => [
            Part::class => [
                // status is filtered on explicitly in PartController as well
                // as via Part::shouldBeSearchable() — belt and suspenders,
                // and it's what keeps the `database` test-driver path (which
                // has no separate index to have excluded draft/discontinued
                // parts from) behaving the same as Meilisearch does.
                'filterableAttributes' => ['category_id', 'manufacturer_id', 'status'],
                'sortableAttributes' => ['base_price_cents', 'name', 'created_at'],
            ],
        ],
    ],
